...
- I want to avoid creating [yet another dataset](https://github.com/Traewelling/line-colors/issues/91) to be integrated with all others, so I will be using OSM as the data
  source as much as I can and add missing information there, if possible.
...
- See https://about.route.lw1.at/en/#linecolor for more information
- A bit WIP so far, but already covering the majority of lines in Austria
...
A lot of routes have colors that are easy to determine, even if they are not stored in OSM or anywhere else. E.g. all buses and trams by Wiener Linien have the same color. So we can check GTFS for all `route_id` that don't have a color yet and use a small number of fixed rules to apply these organization-wide colors.
